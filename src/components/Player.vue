<template>
  ...
  <div :class="['player', getPlayerViewClass()]" v-if="store.state.track">
  ...
  setup() {
    ...
    const toggleDownload = () => {
      if (trackUrl.value) {
        ...
        link.download = store.state.track.name + " - " + store.state.track.artists + ' [320kpbs][Lemonic].mp3'; // Set the desired filename here
        link.click();
    ...
    const toggleView = () => {
      // !=2 for future
      if (PlayerView.value !== 1) {
        PlayerView.value = PlayerView.value + 1
      }
      else {
        PlayerView.value = 0
      }
