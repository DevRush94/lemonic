  ...
  setup() {
    ...
    const getAutocompleteResults = async (query) => {
      if (query) {
        ...
        store.setCurrentPlaylist(localStorage.getItem('currentPlaylist'))
      }
    };
    const getSearchSong = async (query) => {
      ...
      if (response.status === 200) {
        ...
        store.setCurrentPlaylist(JSON.stringify([{ ...data.tracks, tracks: { items: { ...data.tracks.items.map(album => ({ track: album })) } } },]));
      } else {
